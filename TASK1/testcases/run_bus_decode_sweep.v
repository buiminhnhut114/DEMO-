`ifndef RUN_BUS_DECODE_SWEEP_V
`define RUN_BUS_DECODE_SWEEP_V
...
task run_bus_decode_sweep; 
  reg [15:0] r; integer i;
begin
  $display("\n[TC] BUS.Decode/Mapping sweep");
  for (i = 10'h020; i <= 10'h024; i = i + 1) begin
    apb_read(i[11:2], r);
    $display("[%0t] [INFO] READ @0x%03h -> 0x%04h", $time, i, r);
  end
end
endtask
`endif
